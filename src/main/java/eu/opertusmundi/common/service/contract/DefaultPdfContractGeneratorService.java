...
public class DefaultPdfContractGeneratorService implements PdfContractGeneratorService {
    ...
    private float HorizontalOffset(String move, float stringWidth, float pageWidth) {
        float offset = 0;
        if (move.equals("left")) {
    ...
    private Map<Integer, Integer> getLengthPerLine(List<ParsedLine> parsedLines) {
        ...
        for (int i = 0; i < parsedLines.size(); i++) {
            final int    lineNumber = parsedLines.get(i).lineNumber;
            final String text       = parsedLines.get(i).text;
    ...
    private void addBlock(RenderContext ctx, Block block, boolean justify, boolean newPage, boolean endOfSection, int blockNo) throws IOException {
    ...
    private void addHeaderAndFooter(RenderContext ctx, String contractTitle) throws IOException {
		...
		for(final PDPage page : document.getPages()) {
			...
			/* Calculate the widths off all header and footer texts, needed for the calculation of their positions in the page*/
		    final float stringWidthFooter 	= fonts.getFooter().getStringWidth((currentPage+1) + " / " + document.getNumberOfPages())*footerFontSize/1000f;
		    final float stringWidthFooter2 	= fonts.getFooter().getStringWidth(contractTitle)*footerFontSize/1000f;
            ...
            final float centerXHeader = (pageWidth - stringWidthHeader) / 2f;
            final float centerYHeader = pageHeight / 2f;

            /* Calculate the center of the page based on footer */
            final float centerXFooter = (pageWidth - stringWidthFooter) / 2f;
        ...
        try(final RenderContext ctx = RenderContext.of(document, logo, fonts)) {
                ...
                for (final Block block : section.getBlocks()) {
                    for (int j = 0; j < block.getBlockStyles().size(); j++) {
                        if (j > 0 && block.getBlockStyles().get(j).getOffset() == block.getBlockStyles().get(j - 1).getOffset()) {
                            block.getBlockStyles().get(j - 1)
                                    .setStyle(block.getBlockStyles().get(j - 1).getStyle() + "-"
                                            + block.getBlockStyles().get(j).getStyle());
        	...
        	for (final Section section : allSections) {
                ...
                for (int i = 0; i < section.getBlocks().size(); i++) {
    	    		...
    	    		if (section.getBlocks().size() == 1 && (
	    		        block.getText().equals(" ") 	||
	    				block.getText().equals(" ") 	||
	    				block.getText().equals("") 		||
	    				block.getText().equals("\n")
    				)) {
